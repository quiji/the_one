...
func shake(duration, amp, rot_amp, shk_type, shk_mov_type, given_dir=null):
	...
	if not is_processing:
		#start_point = Vector2()
		start_point = current_host.rect_position 
		start_rotation = current_host.rect_rotation
	...
	current_spot = current_host.rect_position
	if given_dir != null:
...
func process(delta) -> bool:
	...
	if shake_delta > 0:
		...
		current_host.rect_position = current_spot.linear_interpolate(target, movement_t)
		current_host.rect_rotation = lerp(current_rotation, target_rotation, movement_t)
		
		shake_t += delta / shake_duration
		movement_t += delta / movement_duration
		return true
	else:
		current_host.rect_position = start_point
		current_host.rect_rotation = start_rotation
		is_processing = false
