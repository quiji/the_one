...
func _ready():
	...
	add_command("exit", self, "_exit", "Quit game")
	add_command("$", self, "_get_node", "Find and load node into a $<name> variable, example:'$my_test Sprite2D'. Nodes must be loaded first in order for them to be manipulated.")
	...
	add_command("show_fps", self, "_show_fps", "Show current fps by running fps on/off")
	add_command("screen", self, "_screen", "max, min, full and unfull screen")
	add_command("time_scale", self, "_time_scale", "set game time_scale")
	
func _input(ev):

	if active and ev.is_action('Console') and ev.is_pressed():
		if $window.visible:
...
func run_command(cmd, att):
	if !commands.has(cmd):
		...
		while i < commands[cmd].size():
			...
			if commands[cmd][i].object.call(commands[cmd][i].method, att):
				out.info("0¡ executed", [cmd])
				out.newline()
			else:
				out.warn("0¡ failed", [cmd])
				out.newline()
			i += 1
...
func _list_methods(args):
	...
	if _var == null:
		...
		var i = 0
		for prop in props:
			if prop.flags & METHOD_FLAG_FROM_SCRIPT != 0:
				i += 1
...
func _show_fps(args):
	if args.size() != 1 or args[0] != 'on' and args[0] != 'off':
		...
		$fps.visible = true
		out.info("Logging FPS...")
	else:
		$fps.visible = false
		out.info("Finished logging FPS.")
	...
	if args.size() != 1:
		...
		match args[0].to_lower():
			"max":
				...
				out.info("Maximizing screen")
			"min":
				OS.window_maximized = false
				out.info("Minimizing screen")
			"full":
				OS.window_fullscreen = true
				out.info("Full screen")
			"unfull":
				OS.window_fullscreen = false
				out.info("UnFulling screen")
			_:
				out.warn("The parameters of the command are not as expected, use the 0¡ command for assistance.", ["help"])
				return false
...
func _time_scale(args):
	if args.size() != 1:
		out.warn("The parameters of the command are not as expected, use the 0¡ command for assistance.", ["help"])
		return false
	else:
		Engine.time_scale = float(args[0])
		out.info("..setting time scale to 0!", [str(args[0])])
